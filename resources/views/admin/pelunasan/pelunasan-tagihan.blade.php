...
@section('content')
    <ul class="nav nav-tabs my-3" id="myTab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="profile-tab" data-bs-toggle="tab" data-bs-target="#tata-cara-pane" type="button"
                role="tab" aria-controls="profile-tab-pane" aria-selected="false">Tata Cara
                Pembayaran</button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link " id="home-tab" data-bs-toggle="tab" data-bs-target="#pembayaran-pane" type="button"
                role="tab" aria-controls="pembayaran-pane" aria-selected="true">Form Upload Pembayaran</button>
        </li>

    </ul>
    <div class="tab-content" id="myTabContent">
        <div class="tab-pane fade show active" id="tata-cara-pane" role="tabpanel" aria-labelledby="tata-cara-tab"
            tabindex="0">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <ol class="list-group list-group-numbered mt-3">
                        ...
                        <li class="list-group-item d-flex justify-content-between align-items-start">
                            <div class="ms-2 me-auto">
                                <div>Upload Bukti Pembayaran Pada Form Upload Pembayaran</div>
                            </div>
                        </li>
                        {{-- <li class="list-group-item d-flex justify-content-between align-items-start">
                        <div class="ms-2 me-auto">
        ...
        <div class="tab-pane fade " id="pembayaran-pane" role="tabpanel" aria-labelledby="pembayaran-pane" tabindex="0">
            <form action="{{ route('pelunasan.lunasi', $tagihan->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-xxl-8 col-xl-8 col-lg-8 col-12">
                    <div class="mb-3">
                        ...
                        <input type="text" id="nama_invoice" name="nama_invoice" class="form-control"
                            value="{{ $tagihan->no_invoice ?? '' }}" readonly>
                    </div>

                    {{-- <div class="mb-3">
                            <label for="total_bayar">Total Bayar</label>
                            <input type="text" id="total_bayar" name="total_bayar" class="form-control" value="{{ $tagihan->biaya->nominal ?? '' }}"
                    readonly>
                </div> --}}
                ...
                @endif
                    ...
                    <div class="mb-3">
                        <label for="total_bayar">Total Bayar</label>
                        <input type="text" id="total_bayar" name="total_bayar" class="form-control"
                            @if ($tagihan->status == 'Kurang') value="{{ $tagihan->biaya->nominal - $tagihan->nominal ?? '' }}"
                @else
                value="{{ $tagihan->biaya->nominal ?? '' }}" @endif
                            readonly>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="upload-area border border-dashed p-4 text-center bg-light">
                            <div class="upload-icon mb-2">
                                <img src="{{ asset('images/upload-icon.svg') }}" id="preview" alt="Upload Bukti"
                                    width="100%">
                            </div>
                            <p>Seret dan jatuhkan file Anda di sini atau <a href="javascript:void(0)" class="text-primary"
                                    onclick="document.getElementById('selectImage').click()">pilih file</a></p>
                            <p class="small text-muted">Format yang didukung: PDF, atau gambar (JPG, PNG)</p>
                            <input type="file" id="selectImage" name="bukti_pembayaran" class="d-none"
    ...
    <div class="d-flex justify-content-between mt-4">
        ...
        <a href="{{ url()->previous() }}" class="btn btn-secondary ms-auto">Kembali</a>
    </div>
@else
    <div class="d-flex justify-content-between mt-4">
